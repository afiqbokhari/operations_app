        ...
        <form action="{{ route('bookings.update', $booking) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="booking_id" value="{{ $booking->booking_id }}" required {{ $viewMode
                        ? 'disabled' : '' }}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white {{ $viewMode ? 'opacity-70 cursor-not-allowed' : '' }}">
                </div>
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Case Reference</label>
                    <input type="text" name="case_reference" value="{{ $booking->case->reference_number ?? '' }}"
                        list="caseList" {{ $viewMode ? 'disabled' : '' }}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white {{ $viewMode ? 'opacity-70 cursor-not-allowed' : '' }}">
                    @if(!$viewMode)
            ...
            $arbitrators = $booking->participants->whereIn('role', ['presiding_arbitrator',
            'co_arbitrator'])->pluck('contact.name')->implode(', ');
            @endphp
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Claimant
                        Solicitor</label>
                    <input type="text" name="claimant_solicitor" value="{{ $claimantSol->contact->name ?? '' }}"
                        list="contactList" {{ $viewMode ? 'disabled' : '' }}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white {{ $viewMode ? 'opacity-70 cursor-not-allowed' : '' }}">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="respondent" value="{{ $respondent->contact->name ?? '' }}"
                        list="contactList" {{ $viewMode ? 'disabled' : '' }}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white {{ $viewMode ? 'opacity-70 cursor-not-allowed' : '' }}">
                </div>
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Respondent
                        Solicitor</label>
                    <input type="text" name="respondent_solicitor" value="{{ $respondentSol->contact->name ?? '' }}"
                        list="contactList" {{ $viewMode ? 'disabled' : '' }}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white {{ $viewMode ? 'opacity-70 cursor-not-allowed' : '' }}">
                </div>
            </div>
            ...
            <div>
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Arbitrators (comma
                    separated)</label>
                <input type="text" name="arbitrators" value="{{ $arbitrators }}" list="contactList" {{ $viewMode
                    ? 'disabled' : '' }}
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white {{ $viewMode ? 'opacity-70 cursor-not-allowed' : '' }}">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Number of
                        Attendees</label>
                ...
                <div>
                    ...
                    <select name="booking_status" {{ $viewMode ? 'disabled' : '' }}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white {{ $viewMode ? 'opacity-70 cursor-not-allowed' : '' }}">
                        <option value="tentative" {{ $booking->booking_status === 'tentative' ? 'selected' : ''
                            }}>Tentative</option>
            ...
            <div x-data="{ breakouts: {{ $booking->breakoutRooms->count() }} }">
                ...
                <div class="flex items-center gap-2 mb-2">
                    <select name="breakout_rooms[]"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        <option value="">Select breakout room</option>
                        @foreach($allRooms as $r)
                        <option value="{{ $r->id }}" {{ $br->room_id == $r->id ? 'selected' : '' }}>{{ $r->room_code }}
                            - {{ $r->room_name }}</option>
                        @endforeach
                    </select>
                    <button type="button" @click="breakouts--; $el.parentElement.remove()"
                ...
                <button type="button" @click="breakouts++"
                    class="mt-2 px-3 py-1.5 text-xs font-medium text-blue-700 bg-blue-100 rounded-lg hover:bg-blue-200 dark:bg-blue-900 dark:text-blue-300 dark:hover:bg-blue-800">+
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <span
                        class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-1 rounded dark:bg-blue-900 dark:text-blue-300">{{
                        $br->room->room_code }} - {{ $br->room->room_name }}</span>
                    @endforeach
                </div>
                @else
            ...
            <div class="flex justify-end space-x-3">
                ...
                <a href="{{ route('bookings.index') }}"
                    class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600">Close</a>
                <a href="{{ route('bookings.edit', $booking) }}"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700">Edit</a>
                @else
                <a href="{{ route('bookings.index') }}"
                    class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600">Cancel</a>
                <button type="submit"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700">Update
                    Booking</button>
                @endif
            </div>
        </form>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-2 hidden md:table-cell text-xs">@if($log->changes) @foreach($log->changes
                                as $change) {!! $change !!}<br> @endforeach @else {{ $log->description }} @endif</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
